...
`baseline` / `same-config` / `config-changed`. Both the built-in text `stability` and your
own `evaluation.metrics` scores are trended (`--metric` focuses on one; `--plot` hands the
series to `evaluation.plots`). For sweeps and CI, turn a comparison or evaluation into an
exit code:
...
a breach. With `embedding_model: none` the built-in verdict is null and never trips; gate
on a project metric instead. dow computes no new numbers — the gate only interprets the
verdict or score already produced.
...
edit, commit, compare loop. `dow_compare` also takes `fail_on` for a structured
regression-gate decision. The server also exposes read-only resources an MCP client
can attach as context: `dow://overview`, `dow://docs/<command>`, `dow://specs`, and
`dow://spec/<name>`.
